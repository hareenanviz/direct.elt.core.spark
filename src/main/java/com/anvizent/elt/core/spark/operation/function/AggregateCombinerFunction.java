 ...
 *
 */
public class AggregateCombinerFunction extends AnvizentFunction2 {

	private static final long serialVersionUID = 1L;

	private HashMap<String, String[]> averageFields;

	public AggregateCombinerFunction(ConfigBean configBean, LinkedHashMap<String, AnvizentDataType> structure,
	...
	public HashMap<String, Object> process(HashMap<String, Object> row_1, HashMap<String, Object> row_2)
		...
		HashMap<String, Object> newRow = new HashMap<String, Object>(row_2);
		...
		try {
			...
			for (int i = 0; i < groupByConfigBean.getAggregationFields().size(); i++) {
				...
				if (aggregationType.equals(Aggregations.SUM)) {
					newRow.put(aggregationFieldsAlias, AggregationService.sum(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.COUNT)) {
					newRow.put(aggregationFieldsAlias, AggregationService.combinerCount(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.COUNT_WITH_NULLS)) {
					newRow.put(aggregationFieldsAlias,
					        AggregationService.combinerCountWithNulls(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.MIN)) {
					newRow.put(aggregationFieldsAlias, AggregationService.min(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.MAX)) {
					newRow.put(aggregationFieldsAlias, AggregationService.max(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.RANDOM)) {
					newRow.put(aggregationFieldsAlias, AggregationService.random(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias)));
				} else if (aggregationType.equals(Aggregations.JOIN_BY_DELIM)) {
					newRow.put(aggregationFieldsAlias, AggregationService.joinByDelim(row_1.get(aggregationFieldsAlias), row_2.get(aggregationFieldsAlias),
					        groupByConfigBean.getJoinAggregationDelimeters().get(i)));
				} else {
					String[] averageFieldsNames = averageFields.get(aggregationFieldsAlias);
					newRow.put(averageFieldsNames[0], AggregationService.sum(row_1.get(averageFieldsNames[0]), row_2.get(averageFieldsNames[0])));
					newRow.put(averageFieldsNames[1], AggregationService.combinerCount(row_1.get(averageFieldsNames[1]), row_2.get(averageFieldsNames[1])));
				}
			}
