...
import com.anvizent.elt.core.spark.sink.doc.helper.S3CSVFileSinkDocHelper;
import com.anvizent.elt.core.spark.sink.validator.S3CSVFileSinkValidator;
 ...
 *
 */
public class S3CSVFileSinkFactory extends SinkFactory {
	...
	protected void write(ConfigBean configBean, Component component, ErrorHandlerSink errorHandlerSink) throws Exception {
		...
		dataFrameWriter.format(FileFormat.CSV.getValue()).save(Constants.Protocol.S3 + s3Connection.getBucketName() + s3jsonFileSinkConfigBean.getPath());
	}

	@Override
	public DocHelper getDocHelper() throws InvalidParameter {
		return new S3CSVFileSinkDocHelper(this);
	}

	@Override
	public String getName() {
		return ConfigConstants.Sink.Components.S3_CSV_SINK.get(General.NAME);
	}

	@Override
	public Validator getValidator() {
		return new S3CSVFileSinkValidator(this);
	}
