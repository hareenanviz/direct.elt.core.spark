 ...
 *
 */
public class RetainFieldsAsCallTest extends AnvizentFunctionMappingPositiveCallTest {

	private Date now = new Date();

	@Override
	public MappingConfigBean getConfigBean() {
		return new RetainConfigBean(getRetainFields(), getRetainFieldsAs(), null, null);
	}
	...
	private ArrayList<String> getRetainFieldsAs() {
		ArrayList<String> retainFields = new ArrayList<>();
		retainFields.add("id");
		retainFields.add("company_name");
		retainFields.add("order_date");
		retainFields.add("country");
		retainFields.add("customer_name");
		retainFields.add("plant_name");
	...
	public HashMap<String, Object> getExpectedValues() {
		...
		expectedValues.put("company_name", "USA");
		expectedValues.put("order_date", now);
		expectedValues.put("country", "UNITED STATES");
		expectedValues.put("customer_name", "VTI OF GEORGIA  INC.");
		expectedValues.put("plant_name", "Statesville NC (1130)");
	...
	public LinkedHashMap<String, AnvizentDataType> getNewStructure() throws UnsupportedException {
		...
		newStructure.put("company_name", structure.get("Company"));
		newStructure.put("order_date", structure.get("OrderDate"));
		newStructure.put("country", structure.get("Country"));
		newStructure.put("customer_name", structure.get("CustomerName"));
		newStructure.put("plant_name", structure.get("PlantName"));
